...
public class DomeProjection : MonoBehaviour
{
    ...
    void Start()
    {
		...
		//Shader shader = Shader.Find("Hidden/SideBySide");
        if (shader != null)
        {
    ...
    void LateUpdate()
    {
        ...
        if (m_cubeRT == null || m_cubeRT.width != cubeMapSize)
        {
            ...
            m_cubeRT = new RenderTexture(cubeMapSize, cubeMapSize, 16, RenderTextureFormat.ARGB32);
			m_cubeRT.isCubemap = true;
            m_cubeRT.Create();     
			//Debug.Log (m_cubeRT.dimension);
        }  
    ...
    void OnRenderImage(RenderTexture src, RenderTexture dest)
    {
        ...
        // Set fisheye FOV.
		m_material.SetFloat("_HalfFOV", ((float) m_controller.DomeAngleProjection) * 0.5f * Mathf.Deg2Rad);
